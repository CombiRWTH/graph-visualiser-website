use crate::{ford_fulkerson, FordFulkersonPseudocode::*, FordFulkersonVisualisationState};
...
fn check_line1(state: FordFulkersonVisualisationState) {
...
fn check_line3(state: FordFulkersonVisualisationState) {
	// Is augmented path determined correctly
	let mut current = 0;
	for &node in state.augmented_path.iter().skip(1) {
		assert!(state
			.residual_graph
			.links
			.iter()
...
fn check_line4(state: FordFulkersonVisualisationState) {
...
fn check_line5(_state: FordFulkersonVisualisationState) {
...
fn test_graph_simple_example() {
	...
	let state = FordFulkersonVisualisationState::new(graph.clone(), 0);
	let sm = StateMachine::new(|out| ford_fulkerson(state, out));
	// Run macro
	test_run!(sm => [
