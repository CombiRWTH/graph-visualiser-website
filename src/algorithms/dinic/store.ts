...
import { config, IDinicConfig, VisualisationStateRS, VisualisationStateTS } from "./config";
import { IAlgorithmStore, LayoutAlgorithm, SetGraphOptions } from "../algorithm-interfaces";
// import { buildFFRSVisState, buildFFVisState, colorAndLayoutGraph, createInitialVisState } from "../ford_fulkerson/store";

// This whole file is basically copy-and-pasted from the ford-fulkerson store. This should be improved (I already tried to do some of it)
...
export interface DinicStore extends IAlgorithmStore {
	/**
	 * Specialise types for Dinic
	 */
	visState: VisualisationStateTS | null;
	config: IDinicConfig;
	getVisState: () => VisualisationStateTS | null;
	setConfig: (config: IDinicConfig) => void;
	resGraphActive: boolean;
}

export const useDinicStore = create<DinicStore>()((set, get) => ({
	isInitialized: false,
	pseudoCode: null,
	visState: null,
	...
	// Automatically switch to the graph that is relevant right now
	nextStep: () => {
		set((state) => {
			const { visState } = state;
			if (visState === undefined || visState === null) return {};

			const newVisState = buildFFVisState(nextStepRS(buildFFRSVisState(visState))) as VisualisationStateTS;

			const updates: Partial<typeof state> = { visState: newVisState };

			if (newVisState.lineOfCode === 2) {
				updates.resGraphActive = true;
			} else if (newVisState.lineOfCode === 4) {
				updates.resGraphActive = false;
			}
	...
	setConfig: (config: IDinicConfig) => set({ config }),
	...
	state.graph = applyColorAndLayout(
		...
		(edge) => {
			...
			if (resGraphActive === true) {
				...
				const flow = state.blockingFlow[keyLinkToString(edge.source, edge.target)];
				if (flow !== undefined && flow > 0) {
					edge.style!.label = { value: flow.toString() + "/" + edge.weight.toString() };
					colorEdge(edge, colors.blockingFlowColor);
				}
			} else if (resGraphActive === false) {
