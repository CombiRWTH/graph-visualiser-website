...
import { config, IMbfConfig, VisualisationStateTS } from "./config";
...
interface MbfState extends IAlgorithmStore {
	/**
	 * Specialise types for Mbf
	 */
	visState: VisualisationStateTS | null;
	config: IMbfConfig;
	getVisState: () => VisualisationStateTS | null;
	setConfig: (config: IMbfConfig) => void;
}
 ...
 */
export const useMbfStore = create<MbfState>()((set, get) => ({
	isInitialized: false,
	pseudoCode: null,
	visState: null,
	...
	setConfig: (config: IMbfConfig) => set({ config }),
...
function colorAndLayoutGraph(
	...
	state.graph = applyColorAndLayout(
		...
		// Edge styler
		(edge) => {
			if (linkInEdgelist(edge, state.usedEdges, true)) {
				colorEdge(edge, colors.usedEdgeColor);
			} else if (
				state.activeEdge !== null &&
				edge.source === state.activeEdge[0].toString() &&
				edge.target === state.activeEdge[1].toString()
			) {
				colorEdge(edge, colors.activeEdgeColor);
			} else if (linkInEdgelist(edge, state.shortestPathTree, true)) {
				colorEdge(edge, colors.shortestPathTreeColor);
			} else {
				const fallback =
