...
import ExplainerPrimMazeFinder from "./ExplainerPrimMazeFinder";

interface cellType {
	status: "blocked" | "passage" | "frontier" | "visited" | "source" | "target" | "shortestPath";
}

export default function PrimMazeFinder(): JSX.Element {
	/* ==== STATES ==== */
	...
	function primOneStep(): void {
		...
		while (i++ < primChunkSizeRef.current && frontierList.length > 0) {
			...
			const passageNeighbours = directions
				...
				.filter((cand) => isInBounds(cand.x, cand.y) && updatedCells[cand.y][cand.x].status === "passage");
			if (passageNeighbours.length > 0) {
				const neighbor = passageNeighbours[Math.floor(Math.random() * passageNeighbours.length)];
	...
	function dijkstraInitialization(): void {
		...
		const predecessors: Array<Array<{ x: number; y: number } | null>> = Array(rows)
			.fill(null)
	...
	function clearMaze(): void {
		...
		visitedRef.current = Array.from({ length: rows }, () => Array.from({ length: cols }, () => false));
		predecessorsRef.current = Array.from({ length: rows }, () => Array.from({ length: cols }, () => null));
	...
	function setHTMLOverlayToCanvas(gridPos: { x: number; y: number }, htmlEl: HTMLElement): void {
		...
		const left = canvasRect.left - containerRect.left + gridPos.x * cellW + cellW / 2;
	...
	useEffect((): void => {
		...
		const sourceElement = document.querySelector<HTMLElement>(".prim-source-node");
	...
	useEffect((): void => {
		...
		const targetElement = document.querySelector<HTMLElement>(".prim-target-node");
			...
			<div className="flex flex-col items-center justify-center">
				...
				<div className="flex-col items-center">
							...
							>
								...
								>
									<rect
										...
										fillOpacity={`${drawingMode === "target" && isMazeComplete ? 1 : 0.25}`}
									/>
								</svg>
							</button>
							<button
					...
					<div className="relative mx-auto flex justify-center">
						<canvas
							ref={canvasRef}
							className="w-[750px] cursor-pointer touch-none select-none"
